...
void writeCommand(unsigned char c) {
    MAP_GPIOPinWrite(GPIOA1_BASE, GPIO_PIN_4 | GPIO_PIN_2, 0x00);
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, c);
    MAP_GPIOPinWrite(GPIOA1_BASE, GPIO_PIN_4 | GPIO_PIN_2, GPIO_PIN_4 | GPIO_PIN_2);
    return;
}
//*****************************************************************************

void writeData(unsigned char c) {
    MAP_GPIOPinWrite(GPIOA1_BASE, GPIO_PIN_2, 0x00);
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, c);
...
*/
  ...
  MAP_SPIConfigSetExpClk(GSPI_BASE,MAP_PRCMPeripheralClockGet(PRCM_GSPI),
                   ...
                   (SPI_HW_CTRL_CS |
                   SPI_4PIN_MODE |
                   SPI_TURBO_ON | // <- Changed turbo to on
                   SPI_CS_ACTIVELOW |
                   SPI_WL_8));

  MAP_SPIEnable(GSPI_BASE);

  GPIOPinWrite(GPIOA1_BASE, 0x10, 0x10);    // DC = High
  GPIOPinWrite(GPIOA1_BASE, 0x08, 0x00);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA1_BASE, 0x08, 0x08);	// RESET = RESET_HIGH
